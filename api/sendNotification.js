...
module.exports = (req, res) => {
	...
	try {
		...
		if(headings)   headings     = JSON.parse(headings);
		if(data)       data         = JSON.parse(data);
		if(buttons)    buttons      = JSON.parse(buttons);
		if(webButtons) webButtons   = JSON.parse(webButtons);
		if(androidBackgroundLayout) androidBackgroundLayout 
								    = JSON.parse(androidBackgroundLayout);
